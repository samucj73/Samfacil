...
# 🔄 Captura dos 300 concursos
with st.spinner("🔄 Buscando últimos 300 resultados da Lotofácil..."):
    concursos = capturar_ultimos_resultados(qtd=300)

if not concursos:
    st.error("❌ Não foi possível obter os resultados.")
    st.stop()

# 🟢 Exibir o concurso mais recente
...
if st.button("🚀 Gerar Cartões Otimizados"):
    with st.spinner("🔍 Gerando cartões com filtros avançados..."):
        ...
        st.session_state.cartoes_gerados = cartoes  # salva para conferência
...
if "cartoes_gerados" in st.session_state:
    st.subheader("📊 Conferência com últimos 300 concursos")
